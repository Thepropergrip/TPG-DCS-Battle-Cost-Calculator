-- ======================================================
-- TPG BATTLE COST COUNTER V1.0 2.5.2026 
-- Drop-in Ready DO SCRIPT FILE / MISSION START Trigger 
-- Works on static objects, ground units, planes, helicopters, weapons (other than small arms bursts and cannon fire such as the M61), naval.
-- Edit/Add Your own unitCosts DB Inside Section Below as needed. 
-- Expanded / Minimal / Hidden / No Display 
-- Visual chart of each coalitions balance of economics.
-- Script Hard ON / OFF Control 
-- Export to DCS.log Option 
-- A word on costs: Prices are in 2026 adjusted for inflation USD. Many items are a bit arbritrary however estimates were often based on current unit pricing. Older units that are still in use today are priced for secondary markets if primary users no longer exist. Feedback on the database or script function welcomed, and feel free to customize to suit your needs! 
--This script is for entertaiment purposes only, contents derived solely from public sources. 
-- ======================================================
...
WeaponTracker.ui = {
    displayMode = "expanded", -- expanded / minimal / hidden / nodisplay
    scriptEnabled = true
}
...
["CHAP-M1083"] = 185000,
["CHAP-M1130"] = 1500000,
["CHAP-M142-ATACMS-M39A1"] = 1100000,
["CHAP-M142-ATACMS-M48"] = 1250000,
["CHAP-M142-GMLRS-M30"] = 5200000,
["CHAP-M142-GMLRS-M31"] = 168000,
["CHAP-MATV"] = 650000,
...
["CHAP_M1083"] = 185000,
["CHAP_M1130"] = 1500000,
["CHAP_M142_ATACMS_M39A1"] = 1100000,
["CHAP_M142_ATACMS_M48"] = 1250000,
["CHAP_M142_GMLRS_M30"] = 5200000,
["CHAP_M142_GMLRS_M31"] = 168000,
["CHAP_MATV"] = 650000,
...
function WeaponTracker:recordWeapon(side, rawName)
    ...
    local cost = getCost(rawName)
    local name = rawName and rawName:gsub("weapons.shells.", "") or "Unknown"

    d.shots[name] = (d.shots[name] or 0) + 1
    d.totalCost = d.totalCost + cost
end


function WeaponTracker:recordLoss(unit)
    if not self.ui.scriptEnabled then return end
    ...
    if not self._countedLosses then
        self._countedLosses = {}
    end

    if unitId and self._countedLosses[unitId] then return end
    if unitId then
        self._countedLosses[unitId] = true
    end

    local rawName = unit:getTypeName()
    local d = self.data[side]
    if not d then return end

    local cost = getCost(rawName)
    local key = rawName or "Unknown"

    d.losses[key] = (d.losses[key] or 0) + 1
    d.totalCost = d.totalCost + cost
...
function WeaponTracker.buildReport()
    ...
    if WeaponTracker.ui.displayMode == "hidden" then
        return ""
    end

    local blueCost = WeaponTracker.data[2].totalCost
    local redCost  = WeaponTracker.data[1].totalCost
    local grandTotal = blueCost + redCost

    local msg = "--- TPG BATTLE COST COUNTER ---\n\n"

    local barWidth = 24
    local bluePct = grandTotal > 0 and (blueCost / grandTotal) or 0
    local redPct  = grandTotal > 0 and (redCost / grandTotal) or 0

    local blueBar = string.rep("█", math.floor(bluePct * barWidth)) .. string.rep("░", barWidth - math.floor(bluePct * barWidth))
    local redBar  = string.rep("█", math.floor(redPct * barWidth)) .. string.rep("░", barWidth - math.floor(redPct * barWidth))

    msg = msg .. "=== ECONOMIC BALANCE OF FIRE ===\n"
    msg = msg .. string.format("BLUE |%s| %d%% $%s\n", blueBar, math.floor(bluePct*100), WeaponTracker.formatNumber(blueCost))
    msg = msg .. string.format("RED  |%s| %d%% $%s\n\n", redBar, math.floor(redPct*100), WeaponTracker.formatNumber(redCost))

    if WeaponTracker.ui.displayMode == "minimal" then
        msg = msg .. "WAR TOTAL: $" .. WeaponTracker.formatNumber(grandTotal)
        return msg
    end

    if WeaponTracker.ui.displayMode ~= "expanded" then
        return msg
    end

    for _, side in ipairs({2,1}) do
        local d = WeaponTracker.data[side]
        msg = msg .. "\n--- " .. d.name .. " ---\n"

        for name, count in pairs(d.shots) do
            local subtotal = getCost(name) * count
            local pct = d.totalCost > 0 and (subtotal / d.totalCost) * 100 or 0
            msg = msg .. string.format("FIRED: %s x%d  $%s  %.1f%%\n",
                name, count, WeaponTracker.formatNumber(subtotal), pct)
        end

        for name, count in pairs(d.losses) do
            local subtotal = getCost(name) * count
            local pct = d.totalCost > 0 and (subtotal / d.totalCost) * 100 or 0
            msg = msg .. string.format("LOSS: %s x%d  $%s  %.1f%%\n",
                name, count, WeaponTracker.formatNumber(subtotal), pct)
        end

        msg = msg .. "\nCOALITION TOTAL: $" .. WeaponTracker.formatNumber(d.totalCost) .. "\n"
    end

    msg = msg .. "\n=== WAR TOTAL === $" .. WeaponTracker.formatNumber(grandTotal)
...
function WeaponTracker:onEvent(event)
    ...
    if event.id == world.event.S_EVENT_SHOT and event.initiator and event.weapon then
        ...
        self:recordWeapon(side, weaponName)
    end

    -- Ground / air unit destroyed
    if event.id == world.event.S_EVENT_DEAD and event.initiator then
        self:recordLoss(event.initiator)
    end

    -- Aircraft kill event (more reliable in modern DCS)
    if event.id == world.event.S_EVENT_KILL and event.target then
        self:recordLoss(event.target)
    end

    -- Crash event fallback
...
function WeaponTracker.refreshUI()
    ...
    if WeaponTracker.ui.displayMode == "nodisplay" then
        return timer.getTime() + 1
    end

    local report = WeaponTracker.buildReport()
    if report ~= "" then
        trigger.action.outText(report, 2, true)
    end
...
function WeaponTracker.setupRadio()
    ...
    for _, side in ipairs({coalition.side.BLUE, coalition.side.RED}) do
        ...
        missionCommands.addCommandForCoalition(side, "Expanded Display", root, function()
            WeaponTracker.ui.displayMode = "expanded"
        end)

        missionCommands.addCommandForCoalition(side, "Minimal Display", root, function()
            WeaponTracker.ui.displayMode = "minimal"
        end)

        missionCommands.addCommandForCoalition(side, "Hidden Display", root, function()
            WeaponTracker.ui.displayMode = "hidden"
        end)

        missionCommands.addCommandForCoalition(side, "No Display (Silent Running)", root, function()
            WeaponTracker.ui.displayMode = "nodisplay"
        end)
...
timer.scheduleFunction(function() return WeaponTracker.refreshUI() end, nil, timer.getTime() + 1)
