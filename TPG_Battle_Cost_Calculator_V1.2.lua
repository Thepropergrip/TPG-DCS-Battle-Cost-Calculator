-- ======================================================
-- TPG BATTLE COST COUNTER V1.2 2.11.2026 
-- Drop-in Ready DO SCRIPT FILE / MISSION START Trigger 
-- Works on static objects, ground units, planes, helicopters, weapons (other than small arms bursts and cannon fire such as the M61), naval. (Ground tracking fixed in this version)
...
-- Top 10 Currency Selection via Radio Menu (Default USD)
...
local function getCost(rawName)
    ...
    if unitCosts[cleanName] then return unitCosts[cleanName], cleanName end
    
    local norm = cleanName:gsub("[%s%-_]", ""):lower()
    for k, v in pairs(unitCosts) do
        local check = tostring(k):gsub("[%s%-_]", ""):lower()
        if check == norm then return v, k end
    end
    return 0, cleanName
end
...
function WeaponTracker:recordWeapon(side, rawName)
    ...
    local displayName = rawName:gsub("weapons.shells.", "")
    
    d.shots[displayName] = (d.shots[displayName] or 0) + 1
    d.totalCost = d.totalCost + cost
end

function WeaponTracker:recordLoss(side, rawName, category)
    if not self.ui.scriptEnabled then return end
    if not side or side <= 0 then return end 
    ...
    local cost = getCost(rawName)
    d.losses[rawName] = (d.losses[rawName] or 0) + 1
    d.totalCost = d.totalCost + cost
end
...
function WeaponTracker.buildReport()
    ...
    for sideID = 2, 1, -1 do
        ...
        for name, count in pairs(d.shots) do
            local costPerUnit = getCost(name)
            ...
            msg = msg .. string.format(
                ...
                count,
                symbol,
                WeaponTracker.formatNumber(WeaponTracker.convert(total)),
                pct
            )
        end
...
function WeaponTracker:onEvent(event)
    ...
    if event.id == world.event.S_EVENT_DEAD or event.id == world.event.S_EVENT_CRASH then
        ...
        if side and side > 0 and category then
            self:recordLoss(side, obj:getTypeName(), category)
        end
    end
end
